...
- 🏷️ **Categoria de investimento com nome consistente** — ao escolher uma categoria já existente no lançamento do mês, a descrição preenche sozinha com o nome exato (evita "emergência" num mês e "Emergência" no outro virarem categorias diferentes); grupos que já ficaram divididos por causa disso têm um "Mesclar em outro nome" na aba Investimentos
- 🧹 **Campo de categoria unificado e automático** — no lançamento do mês, o Tipo vem antes da Categoria; ao marcar Investimento, o próprio campo Categoria vira a lista de categorias da carteira (com foco automático), sem duplicar campo; trocar o Tipo de volta limpa a escolha. Categorias de investimento só se criam na aba Investimentos, nunca mais pelo formulário do mês. Botões "Excluir lançamentos sem categoria" (limpeza dos soltos) e "Excluir tudo" (reset completo da carteira e de todos os lançamentos) na aba Investimentos — ambos reversíveis pelo histórico
- 🏷️ **Nome limpo nas categorias de investimento** — o seletor mostra só o nome (“Caixinha Turbo”), sem prefixar com a classificação (“Outros · ”); a classificação (CDB, Ações…) fica só na coluna própria da tabela da carteira. Botão “Nova categoria” com nome obrigatório
- 💰 **Campos de valor com máscara em todos os formulários** — Metas, Reservas, Investimentos, Patrimônio e Contas fixas ganham a mesma formatação automática (separador de milhar, vírgula decimal) do lançamento do mês, em vez de campo de texto puro
- 📅 **Investimento lançado direto na aba Investimentos conta no mês certo** — aporte inicial (ou valor atual informado, se o aporte ficar zerado) de uma categoria com Data de início no mês atual entra no total de Investimentos do Mês (com linha própria na lista), do Dashboard e do Anual, sem precisar lançar de novo pelo formulário; a Sobra/Saldo continua usando só os lançamentos reais do mês, ja que dinheiro cadastrado na carteira nao é dinheiro que saiu do caixa
...
- 🏦 **Extrato e fatura do banco** — importa OFX, CSV e QIF (Nubank e afins). Fatura de cartão vira itens dentro do lançamento do cartão (mês sugerido pelo nome do arquivo); extrato vira lançamentos no mês de cada data. Duplicados pulados (FITID/Identificador + data/descrição/valor — pode importar OFX e CSV do mesmo período), nomes limpos (Shopee/Amazon/Google com vendedor, parcelas "3/12", anotações entre parênteses preservadas, Pix com nome) e categoria sugerida
